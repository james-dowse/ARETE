...
describe('minPerMov', () => {
  it('30s/série + repos entre séries (rest en secondes)', () => {
    expect(minPerMov(3, 60)).toBe(3.5)      // 3*0.5 + 2*(60/60)
    expect(minPerMov(2, 60)).toBe(2)        // 2*0.5 + 1*(60/60)
    expect(minPerMov(1, 60)).toBe(0.5)      // 1 série → aucun repos
  })
  it('mode durée : la durée exacte remplace les 30s', () => {
    expect(minPerMov(3, 60, 45)).toBe(4.25) // 3*0.75 + 2*(60/60)
  })
})

describe('sizeWorkout — mode « Au temps » ne dépasse jamais la cible', () => {
  // Régression du bug « 55 min → 1h10 ». Repos exprimés en secondes (2min=120, 1min=60).
  it('55 min / Intermédiaire (sets=3) tient dans 55 min', () => {
    const s = sizeWorkout({ targetDur: 55, sets: 3, globalBlockRest: 120, defaultRest: 60, fixed: true })
    const blocks = s.distribution.map(count => ({ count, sets: 3, rest: 60 }))
    const est = estimateTotalMinutes(blocks, 120)
  ...
  it.each([20, 30, 45, 60, 90])('%i min ne dépasse pas la cible', (dur) => {
    for (const sets of [2, 3, 4]) {
      const s = sizeWorkout({ targetDur: dur, sets, globalBlockRest: 120, defaultRest: 60, fixed: true })
      const blocks = s.distribution.map(count => ({ count, sets, rest: 60 }))
      expect(estimateTotalMinutes(blocks, 120)).toBeLessThanOrEqual(dur)
